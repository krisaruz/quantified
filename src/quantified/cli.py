...
def recommend():
    ...
    from quantified.portfolio import load_portfolio
    from quantified.recommender import Recommender, format_recommendation
    ...
    if filtered.empty:
        click.echo("无数据，请先运行 quantified sync")
        return
...
def web(port: int):
    """启动 Web 界面"""
    from quantified.web.app import run_server
